...
def main():
    ...
    pin_bin += str(pin_bin[7:14].count("1") % 2)

    # Conversion en string binaire
    oldpin_bin = bin(int(oldpin))[2:].zfill(14)
    # Ajout du checksum
    oldpin_bin += str(oldpin_bin[:7].count("1") % 2)
    oldpin_bin += str(oldpin_bin[7:14].count("1") % 2)

    code = bytes([int(pin_bin[:8], 2), int(pin_bin[8:], 2), int(oldpin_bin[:8], 2), int(oldpin_bin[8:], 2)]) + timestamp
